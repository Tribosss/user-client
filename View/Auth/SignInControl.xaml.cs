...
{
    /// <summary>
    /// 로그인 화면. 로그인 실패 3회 이상 시 OTP 화면으로 전환.
    /// 로그인 성공 시 SuccessSignInEvt 호출.
    /// </summary>
    public partial class SignInControl : System.Windows.Controls.UserControl
    {
        ...
        private void HandleLoginFailure()
        {
            ...
            {
                // OTP 인증 화면으로 전환
                var totpControl = new OtpControl
                {
                ...
                totpControl.OtpSuccessEvt += () =>
                {
                    ...
                    if (this.Parent is System.Windows.Controls.Panel parent)
                    {
                        ...
                        parent.Children.Add(new SignInControl(SuccessSignInEvt, GotoSignUpEvt));
                    }
                };

                if (this.Parent is System.Windows.Controls.Panel p)
                {
                    p.Children.Clear();
                    p.Children.Add(totpControl);
                }
            }
            else
            {
